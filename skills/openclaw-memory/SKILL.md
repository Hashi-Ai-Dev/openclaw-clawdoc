...
**⚠️ For Honcho: you MUST set `plugins.slots.memory = "openclaw-honcho"`.** Honcho registers its own tools (`honcho_context`, `honcho_search_conclusions`, `honcho_search_messages`, `honcho_session`, `honcho_ask`) AND requires the memory slot to be set to `openclaw-honcho` to function as the memory backend. Leave `slots.memory` at default (`memory-core`) only when using builtin or QMD.
