...
description: "OpenClaw troubleshooting and fixes. Use when diagnosing gateway crashes, config errors that other skills can't resolve, plugin conflicts after config checks pass, ACP agent failures, pairing/device pairing, hook not firing, lock file issues, secret resolution failures, or queue/backpressure problems. For everyday FAQ-style questions use openclaw-help instead. Triggers on: \"config error\", \"crash\", \"debug\", \"troubleshoot\", \"diagnose\", \"not working\", \"failed\", \"error\", \"issue\", \"pairing failure\", \"device pairing\", \"hook failure\", \"lock file\", \"queue\", \"backpressure\", \"secret not resolving\", \"plugin conflict\", \"openclaw doctor\", \"gateway locked\", \"ACP agent won't start\", \"channel silent\", \"hook not firing\", \"debugging tools\", \"hooks\", \"testing kit\", \"troubleshooting flow\", \"diagnostic flowchart\"."
---
...
- **⚠️ `gateway update.run` overwrites config** — `gateway.auth.token` and channel tokens can be invalidated; Discord bot drops immediately after update. Always backup tokens before updating and restart the gateway after.
